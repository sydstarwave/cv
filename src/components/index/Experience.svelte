...
>
	...
	<header class="px-5 relative lg:flex pt-40 pb-24 gap-10 text-center">
		...
		<div data-aos="fade-left" data-aos-delay="200" class="basis-1/2">
			...
			>
				I have more than 10 years of work experience and counting. Here are some companies I have
	...
	<section class="main max-w-6xl mx-auto mb-40 relative z-10">
		<div class="grid grid-cols-1 md:grid-cols-2 mx-6 sm:mx-28 md:mx-0 lg:mx-20 xl:mx-28 shadow-2xl">
			{#each experiences.work as experience}
				<button
					class="exp text-left aspect-square flex bg-gradient-to-br {experience.meta.background}"
				>
					<img
						loading="lazy"
						class="m-auto h-40 sm:h-52 md:h-48 lg:h-40 aspect-square object-scale-down"
						src={experience.meta.logo}
					...
					>
						<h1 class="font-bold text-gray-700 dark:text-white text-xl lg:text-2xl">
							{experience.position}
						</h1>
						<h1 class="font-bold {experience.meta.highlight} text-lg lg:text-lg mb-1">
							@{experience.name}
						</h1>
						<p class="text-gray-500 dark:text-gray-400 mb-5 font-mono text-sm">
							<time>{experience.startDate}</time> - <time>{experience.endDate}</time>
						</p>
						<ul class="text-base font-semibold text-gray-700 dark:text-gray-200 list-square ml-4">
							{#if experience.highlights.length > 0}
								{#each experience.highlights as point}
									<li class="mt-1">{point}</li>
								{/each}
							{:else}
								<li class="mt-1">{experience.summary}</li>
							{/if}
						</ul>
					</div>
				</button>

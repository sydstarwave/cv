...
<article class="sm:mx-14 lg:mx-0">
	<div class="flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-14">
		<aside class="lg:max-w-1/2 w-fit shrink">
			<div
				class="mx-0 mb-4 max-h-80 w-fit bg-gradient-to-br {project.meta
					.background} shadow-xl overflow-hidden"
			>
				<img
			...
			>
				{#if project.meta.github}
					...
					>
						<p class="">Github</p>
						<FontAwesomeIcon icon={faGithub} class="ml-3 text-lg" /></a
					>
				{:else if project.meta.private}
					<button
						class="px-3 py-1.5 dark:bg-gray-800 bg-gray-200 flex items-center text-gray-500 dark:text-gray-400 cursor-not-allowed"
						disabled
					>
						<p class="">Private</p>
						<FontAwesomeIcon icon={faCode} class="ml-3 text-lg" />
					</button>
				{/if}
				{#if project.url}
					<a
					...
					>
						...
						<FontAwesomeIcon icon={faExternalLinkAlt} class="ml-3" />
					</a>
				{/if}
			</div>
		</aside>
		...
		<section class="flex-1 {right ? 'lg:order-first' : ''}">
			...
			>
				{@html project.name}
				<time class="ml-0.5 font-light text-right text-xl lg:text-2xl dark:text-gray-300"
					>{project.startDate}
					{#if project.endDate}- {project.endDate}{/if}</time
				>
			</h1>
			...
			<hr data-aos="fade" data-aos-delay="150" class="mb-4 border-gray-400 dark:border-gray-500" />
			<p
				data-aos="fade"
				data-aos-delay="200"
			...
			>
				{#each project.meta.technologies as tech}
					<a href={techLinks.get(tech)} target="_blank" class="bg-red-200 text-red-900 px-2"
						>{tech}</a
					>
				{/each}
			</div>
  ...
  {#if project.more}
    <article data-aos="fade-left" class="mt-14">
      ...
      >
        <em class="block mb-2"
          ><FontAwesomeIcon icon={faNewspaper} class="mr-2 text-lg" />Featured Article</em
        >
        ...
        <p class="text-gray-500 dark:text-gray-300">
          by Sydney Panesar &nbsp;∙&nbsp; 2 August 2021 &nbsp;∙&nbsp; 8 minute
          read
        </p>
      </a>
    </article>
